...
import { IMAGE_FORMATS, IMAGE_QUALITIES } from "../../src/core/types.js";
...
describe("validateAssetsFile", () => {
  ...
  it("requires at least one asset", () => {
    expect(() => validateAssetsFile({ assets: [] }, SOURCE)).toThrow(
      /at least one asset/,
    );
  ...
  it("names the position of an asset with no id", () => {
    expect(() =>
      validateAssetsFile({ assets: [{ prompt: "x" }] }, SOURCE),
    ).toThrow(/assets\[0\]/);
  });

  it("requires a prompt", () => {
    expect(() =>
      validateAssetsFile({ assets: [{ id: "hero" }] }, SOURCE),
    ).toThrow(/prompt/);
  });

  it("rejects an empty prompt, which would generate something arbitrary", () => {
    expect(() =>
      validateAssetsFile({ assets: [{ id: "hero", prompt: "  " }] }, SOURCE),
    ).toThrow(ConfigError);
  });
  ...
  it("names both positions on a duplicate id", () => {
    ...
    expect(() => validateAssetsFile(input, SOURCE)).toThrow(
      /assets\[0\].*assets\[1\]/s,
    );
  });
  ...
  it("accepts the three named sizes and a literal size", () => {
    for (const name of Object.keys(NAMED_SIZES)) {
      const file = validateAssetsFile(
        { assets: [{ id: "a", prompt: "p", size: name }] },
        SOURCE,
      );
      expect(file.assets[0]!.size).toBe(
        NAMED_SIZES[name as keyof typeof NAMED_SIZES],
      );
    }
    const literal = validateAssetsFile(
      { assets: [{ id: "a", prompt: "p", size: "800x600" }] },
      SOURCE,
    );
        ...
        assets: [
          {
            id: "a",
            prompt: "p",
            variants: [768, { width: 1536, suffix: "@lg" }],
          },
        ],
      },
      SOURCE,
    );
    expect(file.assets[0]!.variants).toEqual([
      { width: 768 },
      { width: 1536, suffix: "@lg" },
    ]);
  });
  ...
  it("rejects a variant with no width", () => {
    expect(() =>
      validateAssetsFile(
        { assets: [{ id: "a", prompt: "p", variants: [{ suffix: "@sm" }] }] },
        SOURCE,
      ),
    ).toThrow(/width/);
  });
  ...
  it.each(["", "   "])("rejects the empty suffix %p", (suffix) => {
    expect(() =>
      validateAssetsFile(
        {
          assets: [{ id: "a", prompt: "p", variants: [{ width: 8, suffix }] }],
        },
        SOURCE,
      ),
    ).toThrow(/suffix/);
  ...
  it.each(["../../outside", "sub/thing", "a\\b"])(
    "rejects a path-shaped suffix %p",
    (suffix) => {
      expect(() =>
        validateAssetsFile(
          {
            assets: [
              { id: "a", prompt: "p", variants: [{ width: 8, suffix }] },
            ],
          },
          SOURCE,
        ),
  ...
  it("fails on a wrong type for a known key", () => {
    expect(() =>
      validateAssetsFile(
        { assets: [{ id: "a", prompt: "p", transparent: "yes" }] },
        SOURCE,
      ),
    ).toThrow(/transparent/);
  });
  // Same guard as config/schema.test.ts: assets.yml must not be the one validator
  // that rejects a value the rest of the tool accepts.
  it.each(IMAGE_FORMATS)("accepts the core format %s", (format) => {
    const file = validateAssetsFile(
      { assets: [{ id: "a", prompt: "p", format }] },
      SOURCE,
    );
    expect(file.assets[0]!.format).toBe(format);
  });

  it.each(IMAGE_QUALITIES)("accepts the core quality %s", (quality) => {
    const file = validateAssetsFile(
      { assets: [{ id: "a", prompt: "p", quality }] },
      SOURCE,
    );
    expect(file.assets[0]!.quality).toBe(quality);
  });
});
