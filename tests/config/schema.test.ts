...
describe("validateConfig", () => {
    ...
    const config = validateConfig(
      {
        ...
        styles: { brand: { palette: "deep navy, warm amber", modifiers: "flat vector" } },
      },
      SOURCE,
    );
  ...
  it("names the key and the file when a type is wrong", () => {
    expect(() => validateConfig({ concurrency: "lots" }, SOURCE)).toThrow(/concurrency/);
    expect(() => validateConfig({ concurrency: "lots" }, SOURCE)).toThrow(/subpixel\.config\.json/);
  });

  it("rejects a format the engine cannot write", () => {
    expect(() => validateConfig({ format: "gif" }, SOURCE)).toThrow(ConfigError);
  });

  it("rejects a concurrency that is not a positive whole number", () => {
    expect(() => validateConfig({ concurrency: 0 }, SOURCE)).toThrow(ConfigError);
    expect(() => validateConfig({ concurrency: 1.5 }, SOURCE)).toThrow(ConfigError);
  });

  it("warns about an unknown key and keeps going", () => {
    const warn = vi.fn();
    const config = validateConfig({ outDir: "img", colour: "blue" }, SOURCE, warn);
  ...
  it("validates style definitions as string maps", () => {
    expect(() => validateConfig({ styles: { brand: "flat vector" } }, SOURCE)).toThrow(ConfigError);
    expect(() => validateConfig({ styles: { brand: { palette: 7 } } }, SOURCE)).toThrow(/palette/);
  });
