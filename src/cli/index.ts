...
export async function buildProgram(): Promise<Command> {
  ...
  program
    ...
    .option("--cache-only", "serve this request only from the cache; exit 7 rather than spend")
    ...
    .option("-b, --backend <name>", "codex-http | codex-exec | auto (default: the config, else auto)")
  ...
  program
    ...
    .option("--global", "write each harness its user-scoped config, for every project")
