...
function asset(overrides: Partial<ResolvedAsset> = {}): ResolvedAsset {
  return {
    ...
    request: {
      ...
      size: "1536x1024",
      format: "webp",
      outputPath: "/repo/public/hero.webp",
    },
    variants: [],
    ...overrides,
  };
...
function probeOver(
  ...
  return {
    ...
    referenceHashes: async (paths) =>
      (paths ?? []).map((path) => hashes[path] ?? "missing"),
...
describe("planAssets", () => {
  ...
  it("treats a file with no manifest as stale", async () => {
    ...
    const statuses = await planAssets(
      [a],
      probeOver(new Set([a.out]), new Map()),
    );
  ...
  it("is stale when the prompt changed", async () => {
    ...
    const changed = asset({
      request: { ...a.request, prompt: "a different dashboard" },
    });
    const statuses = await planAssets(
      [changed],
  ...
  it("is stale when a reference image's contents changed", async () => {
    const a = asset({
      request: { ...asset().request, referenceImages: ["/repo/logo.png"] },
    });
    const manifests = new Map([[a.out, manifestFor(a, ["old-digest"])]]);
    const statuses = await planAssets(
      [a],
      probeOver(new Set([a.out]), manifests, {
        "/repo/logo.png": "new-digest",
      }),
    );
    expect(statuses[0]!.state).toBe("stale");
  });
  ...
  it("is current when every declared variant is present", async () => {
    const a = asset({
      variants: [{ width: 768 }, { width: 1536, suffix: "@lg" }],
    });
    const files = new Set([
      a.out,
      "/repo/public/hero-768w.webp",
      "/repo/public/hero@lg.webp",
    ]);
    const statuses = await planAssets(
      [a],
      probeOver(files, new Map([[a.out, manifestFor(a)]])),
    );
    expect(statuses[0]!.state).toBe("current");
  });
  ...
  it("redacts a credential that reached a prompt", async () => {
    ...
    const statuses = await planAssets([a], probeOver(new Set(), new Map()));
    expect(statuses[0]!.reason).not.toContain(
  ...
  it("keeps input order", async () => {
    const statuses = await planAssets(
      [
        asset({ id: "b", out: "/out/b.png" }),
        asset({ id: "a", out: "/out/a.png" }),
      ],
...
describe("hasDrift", () => {
  it("is false only when everything is current", () => {
    expect(
      hasDrift([
        { id: "a", out: "/a", state: "current", reason: "", key: "k" },
      ]),
    ).toBe(false);
    expect(
      hasDrift([{ id: "a", out: "/a", state: "stale", reason: "", key: "k" }]),
    ).toBe(true);
    expect(
      hasDrift([
        { id: "a", out: "/a", state: "missing", reason: "", key: "k" },
      ]),
    ).toBe(true);
...
describe("planAssets with verify", () => {
  ...
  it("stays current when the bytes do match", async () => {
    ...
    expect((await planAssets([a], probe, { verify: true }))[0]!.state).toBe(
      "current",
    );
  });
  ...
  it("catches a variant whose bytes no longer match its manifest record", async () => {
    // A variant is a file nothing in the cache key describes, so `exists` is the
    // only thing that ever looked at it. Truncate it and every --check passed.
    const a = asset({ variants: [{ width: 800 }] });
    const variant = "/repo/public/hero-800w.webp";
    const manifest: ManifestEntry = {
      ...manifestFor(a),
      variants: [
        { width: 800, height: 533, path: variant, bytes: 99, sha256: "cafe" },
      ],
    };
    const probe = probeOver(
      new Set([a.out, variant]),
      new Map([[a.out, manifest]]),
      {},
      {
        [a.out]: "deadbeef",
        [variant]: "not what was written",
      },
    );
    ...
    const verified = await planAssets([a], probe, { verify: true });
    expect(verified[0]!.state).toBe("stale");
    expect(verified[0]!.reason).toMatch(/hero-800w\.webp.*sha256/);
  });

  it("stays current when a variant's bytes match", async () => {
    const a = asset({ variants: [{ width: 800 }] });
    const variant = "/repo/public/hero-800w.webp";
    const manifest: ManifestEntry = {
      ...manifestFor(a),
  ...
  it("takes a sidecar written before variants were digested on trust", async () => {
    // No sha256 on the record is an older manifest, not a mismatch. Failing here
    // would report drift on every asset generated before this field existed.
    const a = asset({ variants: [{ width: 800 }] });
    const variant = "/repo/public/hero-800w.webp";
    const manifest: ManifestEntry = {
      ...manifestFor(a),
      variants: [{ width: 800, height: 533, path: variant, bytes: 99 }],
    };
    const probe = probeOver(
      new Set([a.out, variant]),
      new Map([[a.out, manifest]]),
